...
def get_classes_to_ids(json_file=os.path.join(os.getcwd(), "data",                                               "camera_lidar_semantic", "class_list.json")):
...
def merge_data(start_index=None, stop_index=None):
    ...
    sub_folders_all_dir = [sub_folder for sub_folder in sub_folders if                     os.path.isdir(os.path.join(data_folder,sub_folder))]
    sub_folders_filtered = [sub_folder for sub_folder in sub_folders_all_dir if               "lidar" in os.listdir(os.path.join(data_folder,sub_folder)) and
    ...
    pickle_outfile = os.path.join(os.getcwd(), "data",                                   "dataset_pc_labels_camera_start_{}_stop_{}.pkl".format(start_index, stop_index))
...
def test_id_consistency(D, index):
    ...
    print("TEST if zero (zero indicates correct behavior): {}".format(                                np.sum(np.subtract(labels_pc, np.array(classes)))))
...
def create_and_export_data(start_indices = [i*2000 for i in range(0, 14)],                            stop_indices = [i*2000 for i in range(1, 15)]):
...
def test():
    ...
    pickle_outfile = os.path.join(os.getcwd(), "data",                                       "dataset_pc_labels_camera_start_{}_stop_{}.pkl".format(0, 3))
