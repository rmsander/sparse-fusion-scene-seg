...
GAMMAS = [1,10,20,30,40]
EXPERIMENT_HEADER = "10000_points_ALL_classes_FOCAL_LOSS_{}"

# Class threshold for weighted cross-entropy
THRESHOLD = 0 # Not needed
...
def main(args, GAMMA, pretrain_model_path=None):
    ...
    '''CREATE DIR'''
    experiment_dir = Path('./experiment/{}'.format(EXPERIMENT_HEADER.format(GAMMA)))
    ...
    for epoch in range(epochs):
        ...
        if test_metrics['accuracy'] > best_acc:
            ...
            print("HERE")
            save_path = '%s/%s_%.3d_%.4f.pth' % (checkpoints_dir,model_name, epoch, best_acc)
...
if __name__ == '__main__':
    ...
    args = parse_args()
    for GAMMA in GAMMAS:
        print("Iterating through gamma: {}".format(GAMMA))
        main(args, GAMMA)
